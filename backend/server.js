...
io.on('connection', socket => {
  ...
  socket.on('cast-vote', data => {
    ...
    if (result.error) {
      logger.warn('Vote casting failed', {
        gameId,
        sessionId,
        error: result.error,
      });
  ...
  socket.on('reveal-answer', data => {
    ...
    if (result.error) {
      ...
      socket.emit('error', {
        error: result.error,
        code:
          result.error === 'Game not found' ? 'GAME_NOT_FOUND' : 'UNAUTHORIZED',
      });
      return;
    }
    ...
    logger.info('Answer revealed successfully', {
      gameId,
      correctAnswer: result.correctAnswer,
    });
  });
