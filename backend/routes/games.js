...
router.post('/', upload.single('photo'), async (req, res) => {
  try {
    ...
    const invalidStatements = parsedStatements.some(
      stmt => typeof stmt !== 'string' || stmt.trim().length === 0
    );
  ...
  try {
    ...
    res.json({
      ...
      game: {
        ...
        votes: game.revealed ? game.votes : [0, 0, 0], // Hide actual votes until revealed
        revealed: game.revealed,
        correctAnswer: game.revealed ? game.correctAnswer : undefined,
        totalVotes: game.revealed ? (game.votes[0] + game.votes[1] + game.votes[2]) : 0, // Hide total until revealed
      },
    });
  } catch (error) {
    res.status(500).json({
